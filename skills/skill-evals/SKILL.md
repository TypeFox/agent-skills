...
Both gates apply no matter where you enter. Always clear Gate 1 (resolve `SKILL_CREATOR`). When you enter at Activity 3 or later against **pre-existing** run outputs, you can't re-run an isolation that already happened — so audit it instead: check each run directory for `access-log.md` or sandbox evidence and confirm it against the run's allowlist. If isolation can't be verified for a run, flag that run to the user before grading rather than silently trusting it.
