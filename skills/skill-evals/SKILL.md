...
**Done when** you have chosen an isolation mechanism (allowlist, trace, or self-report), written the per-run allowlists, confirmed subagents work, and cleared the project-memory channel (no root instruction files on disk, none injected into your own context) — or switched to the copy-paste fallback and told the user.
