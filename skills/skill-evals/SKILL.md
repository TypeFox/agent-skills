...
**One channel no tier catches: harness-injected project memory.** Harnesses inject the repo's root instruction files (AGENTS.md / CLAUDE.md and their hierarchy) directly into subagent context at spawn — in Claude Code this reaches every custom/general subagent and every `Workflow`-spawned agent (only the read-only built-in Explore and Plan agents skip it), and no opt-out exists. The injection is not a tool call, so sandbox records, transcript traces, and access logs all come back clean while every run silently carries repo instructions that bias it — repo conventions at best, and at worst notes about the eval fixtures themselves ("files under evals/ are intentionally broken"), an answer key handed to the baseline. The content is cached when the orchestrating session starts: moving the files aside mid-session does not help (verified — a subagent spawned while both files were off-disk still quoted them verbatim). The check is cheap: when the eval process starts, look for root instruction files (AGENTS.md, CLAUDE.md, or tool-specific equivalents). If they exist, stop and have the user remove them and start a fresh eval session — they are restored from git afterwards (`git checkout -- AGENTS.md CLAUDE.md`). One caveat closes the cache loophole at no extra cost: absent files prove nothing if the session started before they were removed, so also confirm your *own* context carries no injected root-file content — if it does, this session predates the removal and must be restarted. Record the check's outcome alongside the rest of Gate 2's clearance.

**Escape hatch:** restarting is not the only remedy — the **out-of-process headless** run mechanism below sidesteps the cached injection entirely, so a contaminated orchestrator can still run the eval; see the run-mechanism choice for how and for the conditions that keep it honest.
...
Two run mechanisms can satisfy the gate — choose one explicitly and record the choice with the rest of Gate 2's clearance:

- **In-process subagents** (a Task / subagent tool that spawns fresh-context children, or Claude Code's `Workflow` tool): each run is one subagent, scoped or logged as above. The default when the orchestrating session started clean: it gives you parallel fan-out in one turn and, through `Workflow`, cached targeted reruns (see `references/claude-code-workflow-tool.md`). Its limits are the flip side of being in-process: every child inherits the session's cached project memory (the channel above — prevention only, never verifiable), and `Workflow` surfaces no trustworthy per-run token count.
- **Out-of-process headless sessions** (Claude Code: `claude -p`; Codex: `codex exec`): each run is its own fresh process, which assembles project memory from disk at its own start — so with the root files off disk nothing is injected, whereas in-process children always inherit the orchestrating session's cached copy. That makes this the **escape hatch** for a contaminated orchestrator (the only option short of restarting), and unlike the in-process flavor the project-memory channel is *verifiable*, not merely prevented: injected content would appear verbatim in the first user turn of each child's own persisted transcript — for Claude Code under `~/.claude/projects/` (keyed by the run's working directory), the same transcript the tier-2 trace audit reads — so grep the child transcripts for distinctive root-file content and record the result with Gate 2's clearance. Two conditions keep the guarantee honest: the files must be absent along each child's entire cwd→root walk *and* at the user level (e.g. `~/.claude/CLAUDE.md`), and they stay off disk until the last child — retries and graders included — has finished. Headless is a legitimate choice from a clean session too: pick it when the cost side of the delta matters (`claude -p --output-format json` returns harness-measured tokens, duration, and cost for `timing.json` — which `Workflow` cannot) or when runs need a different model (`--model`). Its costs: you script the fan-out and retries yourself, and there is no cache/resume for cheap targeted reruns.
- **Neither available** (no subagent tool, no headless CLI): do **not** quietly run the prompts inline — your context already holds the skill, so an inline "baseline" is fiction. Instead, stop the automated loop and hand the user a fallback: generate the full set of run prompts (one block per run, contents in Activity 2) for them to paste into separate fresh sessions, then collect the outputs into the run directories and resume at grading.

**Done when** you have chosen an isolation mechanism (allowlist, trace, or self-report), written the per-run allowlists, chosen and confirmed the run mechanism (subagents spawn, or the headless CLI runs), and cleared the project-memory channel (no root instruction files on disk, none injected into your own context — or the escape hatch taken) — or switched to the copy-paste fallback and told the user.
