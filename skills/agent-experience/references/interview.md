...
- **Design-doc candidates**: "Where does design rationale live outside the repo — wiki, shared docs, issue threads? Which of those still describe reality?" (A tracker the agent can reach is scanned rather than asked about — the history-scan offer below.) Every named source becomes a rescue candidate with provenance and an `unverified` trust label until checked. And: "Which system do agents or new teammates keep getting wrong at the mechanism level — breaking a deliberate invariant, refactoring away an intentional structure?" Recommend from the audit (recurring review comments, reverted agent changes); a named system with that evidence is a design-doc candidate under the triggers in `docs-structure.md`.

## History-scan offer *(retrofit)*

The git history — and the issue and change tracker, where a fetch path exists (`gh` for GitHub issues and pull requests, an MCP connector; the reachability rule is *repo-local or nonexistent* in SKILL.md) — holds the *why* behind much of what the audit found: the commit that introduced a deviation, the revert that settled a contested design, the issue closed as by-design, the PR thread where an alternative was rejected. All of it is reachable in principle and read by no session in practice, so for an agent it is as invisible as a wiki. Convention archaeology (`audit-playbook.md`, pass 5) reads history for *patterns*; this scan reads it for *decisions*, and it runs only on the user's confirmation, asked in the rounds like any other question, for three reasons: it is the audit's most expensive read; the tracker is an external system read with the user's credentials; and what surfaces widens the deliverable with new durable artifacts. It is a scope-and-access question, not a preference poll: the user decides whether the budget is spent and what may be read, never what the artifacts look like.

- **Ask it in the first round**, from a cheap probe — history depth (`git log --oneline | wc -l`, first and last commit dates), a tracker fetch path (`gh auth status`, a one-item `gh issue list`) and its rough volume — with a bound in the recommendation: "Six years of history and 400 closed issues reachable through `gh`; the audit found four deviations and one contested design with no rationale in the docs. Shall I scan history and tracker for the decisions behind them — recommended: yes, bounded to the last three years and the twenty longest issue threads?" An unbounded scan of a large history consumes the session, so the bound — a time window, paths, a top-N by thread length — is part of the yes. Offer only what the probe found: a copy without history, or no fetch path to a tracker, is a stated finding, not a question; recommend *no* or *later* where the history is shallow or the audit found nothing that lacks a rationale. A declined or deferred scan goes on the Phase 6 roadmap as a re-audit item.
- **Run the confirmed scan as an in-flight lookup** (core protocol), letting only the triage questions that depend on it wait. In git history, read the commit that introduced each deviation (`git log -S'<token>' --reverse -- <path>` — its message is the closest thing to a rationale the repo holds), reverts and re-lands (a contested decision, and which side won), deleted directories and abandoned approaches (`git log --diff-filter=D --summary`), and commit bodies that argue — *because*, *instead of*, *workaround*. In the tracker, read issues closed as by-design or won't-fix (bug-vs-intended adjudication), recurring issues on one capability, PRs whose description or review thread settled a design question, pinned issues and discussions. Stop at the bound and report what stayed unscanned as roadmap material; on a large history this is read-only research and a natural subagent slice — its report is prose, not evidence (`audit-playbook.md`, scaling with subagents). Tracker content that must not land in the repo — private reports, personal data — stays a pointer under the external-pointer rule (`docs-structure.md`).
- **Findings return as frontier questions**, each a candidate with provenance — commit hash, issue or PR URL — and its recommendation: "commit a1b2c3 (2023) replaced X with Y because Z — is that still the reason?" A confirmed finding lands through the theme→destination map on the user's words; an unconfirmed one is rescued with provenance and `unverified` (`docs-structure.md`, Rescue at retrofit). Tracker findings that carry spec evidence — by-design closures, recurring disputes on one capability — are attached to the candidates in the triage above.
