...
2. **Commands** — exact, copy-pasteable, verified by execution: install, dev (with ports and prerequisite services), test, **single-test**, lint, typecheck, build, migrate — with timings where they change behavior ("full suite ~4 min; prefer single-test while iterating"). This is the single highest-leverage section: listed commands get used ~160× more than unlisted ones. Name non-obvious tooling choices explicitly (uv not pip, bun not npm, just not make). Where a task runner fronts the verbs, cite its targets (`make test`, never the raw `python3 -m unittest …` underneath) — and when an entry point changes, update every doc that cites it, so exactly one documented way to run each verb survives.
...
5. **Conventions not already enforced by tooling.** If the linter catches it, don't restate it. Pair every prohibition with its concrete alternative: "don't instantiate HTTP clients directly — use the shared `apiClient` from `lib/http`". Bare don'ts make agents cautious and over-exploratory; files with 15+ unpaired prohibitions measurably degraded output. A second licensed pairing is the **incident-traced guard**: a prohibition paired with the observed failure it fences — including failures of *reasoning*, not just of action ("don't invent constraints these docs don't state — a past session assumed migrations were forbidden and worked around the schema instead of extending it"). The incident is the pairing: it carries the why and proves the line passes the litmus test. Such lines typically arrive through the steering loop's coupling (SKILL.md), added in the same change that fixed the incident. What stays excluded is the bare, sourceless don't.
6. **Boundaries and definition of done** — files never to touch, generated dirs, migration rules, secrets policy ("never commit secrets" is the single most common genuinely helpful constraint), destructive-command rules; what proof is required before claiming completion; and the **change-coupling rules** — the artifact each kind of change must be accompanied by (bugfix → regression test, new feature → test + spec entry, API change → doc update, design change → its design doc amended, recurring agent mistake → the guide or sensor that prevents it, in the same change — the steering loop's coupling from SKILL.md). Couplings are how a project's process rules become explicit and binding for agents — historically they lived in reviewers' heads and were enforced inconsistently. Phrase each as a checkable completion requirement ("done means a test covers the new behavior"), never hedged advice ("might need updating"), and treat each as a sensor-promotion candidate once violations recur. Include the self-maintenance rule: a doc–reality discrepancy encountered mid-task is fixed in the same change (or explicitly flagged), never silently worked around — this is what keeps the map alive between doc-gardening runs. Two further candidate lines for agent-heavy repos: **failure visibility** — a failing check or step is reported visibly (non-zero exit, explicit finding), never silently skipped, and a missing prerequisite is itself a finding to report — and, where agents write tests, the source-inspection boundary defined under convention tests in `techniques.md`: source-inspection tests for structure only, never for behaviour.
7. **PR/commit conventions** — title format, conventional commits, what to exclude.
8. **Pointers** — into docs/: ARCHITECTURE.md, the key active ADRs ("do not contradict accepted ADRs: ADR-0012 …"), the exec-plans protocol where the repo uses one (multi-session work gets a plan in `active/`; completion moves it to `completed/` — without this line, domain sessions never feed the memory system), the product-specs index where the repo keeps behaviour specs (specs adjudicate bug vs. intended, and an unrouted spec has no readers), the design-docs index where the repo keeps design docs (the same routing rule applies), and available skills. This is the map function: one line each, no summaries of what the target says. A pointer may carry a condition — "before any UI work, read `docs/design-guidelines.md`" — so context cost scales with the task.
