...
**3. Affordances — what does this codebase make cheap?** (Defined in SKILL.md.) Start with what the codebase affords (in legacy code, usually maintainability sensors) and improve affordances incrementally; commit to a topology ("CRUD service on JVM", "data dashboard in Node") early — variety reduction. Favor dependencies that can be fully internalized and reasoned about in-repo — "boring" technology models better; sometimes a small, well-tested in-repo reimplementation beats an opaque library.
...
- **Getting the agent to actually run sensors** — a known weak point with two faces; set expectations accordingly rather than promising compliance. Agents skip checks: the honest options, in increasing reliability, are an AGENTS.md instruction or skill (easiest, empirically unreliable — agents skip it and run tools directly); agent hooks (e.g. after file edits); git pre-commit hooks (a reliable forcing point for small-commit workflows); a custom runtime extension. And agents *read* check scripts and predict their results, trading visible failures for invisible ones (*run it, don't read it*): mitigate with instructions that mandate execution with captured output ("evidence-backed verification"), and — where it really matters — compiled or opaque check binaries the agent can only run. Scope the mandate to *each relevant check, once*: blanket "always verify before claiming done" phrasing adds no evidence and invites redundant re-runs of already-green work.
...
- **Instruction cost accounting.** Instructions are not free (the instruction-cost evidence in `agents-md.md`). Periodically re-run the litmus test over AGENTS.md and prune.
...
- **Security posture.** Skills and agent-readable docs are an attack surface: docs are a prompt-injection channel, and pre-commit secret scanning protects against the agent itself. Third-party skills get the dependency treatment (the skills entry above).
- **Re-grade from evidence.** Readiness scores stay diagnostics (`audit-playbook.md`), and scores the system assigned itself drift optimistic: re-grade from the actual code and captured output, never by carrying the previous grade forward.
