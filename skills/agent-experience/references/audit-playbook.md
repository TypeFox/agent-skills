...
- **Claims vs. enforcement.** For every rule stated in docs or agent files: does a sensor enforce it? Prose-only rules are **promotion candidates**, triaged in Phase 3.
...
3. **Rules-without-sensors triage.** For each prose-only rule from Phase 1b, decide: promote to a lint or structural test (mechanical and worth its upkeep — standard 8 in SKILL.md), keep in AGENTS.md or a skill (judgment-laden, or not worth a check), or drop (fails the litmus test). Every kept rule pairs the prohibition with the concrete alternative.
...
- **Verification litters.** Find the caches and build artifacts your runs created by searching the whole delivered tree — a root listing shows only the top-level ones — and remove them; if no `.gitignore` covers them, that gap is itself an AX finding to fix or surface.
