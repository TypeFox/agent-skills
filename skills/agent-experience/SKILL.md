...
3. **Every line passes the litmus:** *would removing it cause a mistake the agent wouldn't otherwise make?* Instructions are not free — single-study numbers, but the best we have: unnecessary rules cost 14–22% extra reasoning tokens, commands listed in AGENTS.md get used ~160× more than unlisted ones, and LLM-generated overview files *reduce* task success while raising cost ~23%. Deviations from ecosystem defaults earn lines; defaults never do.
