...
Processing rewrites AI-generated text toward the author's voice, as the step *before* the author's own polish, never as a replacement for it. It reads the author's DB and the skill's AI DB (`data/ai-style-patterns.json`) — one of them only on explicit request (Single-DB runs, below) — and never writes either. The mode rules, DB lookup, and strictness settings are in SKILL.md. A process run needs no further reference: `measure` prints each row's description, and the pattern fields a rewrite reads are few — `description`, `kind` (`presence` or `absence`; on the DB header the same word says `user` or `ai`), the counter (`regex` or `stat`, minus `exclude`), `rate`, `range` (the per-document band), `spread` (the share of documents showing the habit), `tier` (evidence strength; `tier_override` wins), `evidence` (verbatim quotes), `instead` (what the author uses where an absent construction would sit), `displaces` (the family members the author never uses in a presence pattern's slot), `register_scope` (the registers a pattern is confined to), and `covered_by` (AI-DB rows only: the author-side markers for the same construction); the report also quotes the header's `db_version`, `review.status`, and `corpus.register_weights`. A pattern with no counter is *judged*: counted by reading. An absence row with a few residual corpus hits is *near-absent*, tier 2 by rule (`kind` in db-schema.md). Full definitions are in [db-schema.md](db-schema.md), the dimensions in [taxonomy.md](taxonomy.md) — open them when something is unclear, not as a matter of course.

SKILL.md's two founding rules govern every edit: the author's DB has veto power — where the input already matches the author, nothing moves, even when the AI DB flags the construction as machine-typical — and every rewrite targets the author's *rate*, never zero and never "always", which is the guard against caricature. Processing moves the coordinates that sit outside the author's region, each in whichever direction it is off. Taking out what the machine put in is only half of that; the other half is bringing in what the author would have written instead, because a draft stripped of its machine tics still does not sound like anyone in particular. The two halves appear throughout as *remove* rows and *add* rows.
...
Two variants read one DB instead of two, each only when the request asks for it — by name, or by the choice SKILL.md puts to a user with no profile. Every step below runs as written; this is all that changes.

**AI-only** strips machine habits from a draft without moving it toward any voice: `measure` with `--db data/ai-style-patterns.json` alone, no profile located or checked. With nothing to veto and nothing to bring in, the rewrite rows are removals only — the AI rows `measure` marks `[no author row]`, which with no profile loaded are all the machine-typical ones, plus any built-in `ai_*` counter with a striking value — and there are no add, lean, or do-not-touch rows. A row whose description calls the construction register- or model-split (contractions, heading case) is neutral: the input stays inside the machine's region whichever way it moves. There is no tone brief (tone chooses among a profile's attested forms; report a tone request as unsupported) and nothing for the setting to gate (tiers keep an author's weakly evidenced habits from being applied, and this run applies none). Every rule takes Step 3's no-replacement form — remove where the sentence survives removal, otherwise flag — with a target under half the machine's rate, where the row re-reads `low`, `absent`, or `too-short`; not zero, because a colon or a dash below the machine's rate is nobody's tell. `--report-table` prints these rows with the machine's rate in the rate column.

**Profile-only** polishes the author's own hand-written text for consistency with their profile: `measure` with `--db PROFILE` alone. The AI DB is not read and the built-in `ai_*` counters are no evidence either — the text is the author's, so machine-typicality is not the question — which leaves the AI-evidence column only the author's absence rows (`high` by rule) and makes every other remove row low-confidence: worked at tier 1 (tier 2 in the hard setting), otherwise left for the manual pass, exactly as Step 2 says. Nothing else changes.
...
**Gap** is the size of the edit a row asks for: how far the input sits from the author's rate, counted in occurrences of *this* document — ten sentences short of their short-sentence share, six em dashes to take out. Rows a rewrite has no business touching read 0, so the work is everything above them. It is a per-row number and never a document score — a document has no single distance to the author (SKILL.md).
...
| add (`absent`, or `gap` below the range) | bring the habit in toward the low end of the range — by substitution wherever the family already appears, by manufacture only for a tier-1 pattern in a slot another rewrite opened (both terms: Step 3) |
...
**Substitution and manufacture.** There are two ways to close an add row, and only one of them risks caricature. *Substitution* is switching a family member the input already uses for the one the author uses — `-ize` where the author writes `-ise`, *for example* where they write *for instance*, *a number of* where they write *a bunch of*, *Therefore* where they write *So* — and everything else that adds no content: re-punctuating a clause into a parenthetical aside, contracting a verb, splitting or joining sentences, swapping a connective for one the pattern's `instead` or `displaces` names, moving where a sentence puts its weight. It adds nothing to the text because it lands in a slot the input already offers, filled in someone else's voice, or in one a removal has just opened (the `So` that takes the vacated `However`'s place is a substitution; the same `So` opening a sentence that had no connective is not). So it applies wherever the family appears, at any tier the setting allows, and on `lean` rows too: a `low` verdict is no reason to leave the machine's member standing. The pattern's `displaces` list names the forms to substitute out; where it is empty, the evidence quotes show the author's form and reading the input finds its counterpart. *Manufacture* is new *content* — a claim, a number, an incident, an example, an opinion the input did not carry — and, short of that, an occurrence created where the input offers no slot: dropping a "So" opener into a paragraph that never pivots. That is what turns a voice into a parody of itself, so a manufactured occurrence takes a tier-1 pattern and a slot some other rewrite has opened anyway, and manufactured content is never licensed at all (Step 4). The strictness setting does not move this line: it decides which rows are eligible, not how hard an eligible row is worked, so a soft pass works its tier-1 add rows with every slot the input gives it and reports the shortfall only when the slots run out. A light touch is a shorter list of rows, never a lighter hand on each.
...
How much report it carries follows the length of the input, not where it lives. A document gets the full report below. A short input — a note or a message rather than a document, where most of the profile's presence patterns came back `too-short` and the rewrite rows fit in a line — gets one line instead, with the report itself available on request: a twelve-row table under a rewritten chat message is noise, and noise is what stops an author reading the handover at all, while nothing is hidden — the dropped rows, the `too-short` rows, the judgment calls are all there to give when asked. Where the rewrite rows run past what a line can name, it is a document for this purpose, whatever its `too-short` count says:
...
Setting: <soft|medium|hard>; tone: <brief in one line, or "none — profile rates">; profile: <path>, db_version <n>, review <status> — or "none, AI-only".
...
- <pattern>: <why — tier above setting / low confidence / out of the input's register / would have needed manufacture / no replacement in the DB / an AI row with no author row to veto or target it>
...
Extraction has its own parallel protocol: "Parallel extraction with subagents" in [technique.md](technique.md).
