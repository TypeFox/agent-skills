...
| `documents` | per-document evidence: a map from document id to one number, the unit's own — under `per_1k_words` the raw occurrences, an integer; under every other unit the per-document value in that unit. A per-1k statistic (`list_items_per_1k`) is filed under `per_1k_words` with the raw item count; `validate` rejects a value of the wrong type. `styledb.py count` writes the map from the corpus. The numbers are measurements, not estimates: for a `counted` pattern `validate --corpus-dir` re-runs the counter over each document and reports a value the corpus does not reproduce, because everything derived — `rate`, `range`, `spread`, and through them the tier — is computed from them. One key per document *measured*; a document with no key was not measured for this pattern, which lowers coverage. For a judged pattern a `0` asserts that the document was read in full for this habit and shows none of it — a document not read for it gets no key |
