...
| `corpus.documents[].id` | stable slug: every pattern's `documents` map is keyed by it, and every `evidence[]` entry names it |
...
```json
{
  ...
  "stat": "em_dash",
  "unit": "per_1k_words",
  "documents": {"blog-fs-mocks": 0, "email-release": 0},
  "evidence": [],
...
| `kind` | `presence` (the author does this) or `absence` (the author does not — the do-not-introduce list). An absence tolerates a few residual hits and is then *near-absent*: under 0.1 per 1k measured words, in at most a fifth of the measured documents (`NEAR_ABSENCE_MAX_RATE` and `NEAR_ABSENCE_MAX_SPREAD` in `styledb.py`; per-1k unit only). Any more and the row is a presence — `validate` enforces the line — and what the hits are belongs in `note` for the review round. A *presence* row whose numbers land inside the same tolerance only earns a warning: the presence tier rules below read that rarity as weak evidence (tier 3, out of scope on the medium setting) while the same numbers under `absence` are near-absent and tier 2, so which side a rare habit belongs on is the author's call — review-round material (Step 5 of [technique.md](technique.md)), never a `--fix` |
...
| `documents` | per-document evidence: a map from document id to one number, the unit's own — under `per_1k_words` the raw occurrences, an integer (a per-1k statistic such as `list_items_per_1k` is filed there with the raw item count); under every other unit the per-document value in that unit. `validate` rejects a value of the wrong type. `styledb.py count` writes the map from the corpus, and for a `counted` pattern `validate --corpus-dir` re-runs the counter over each document and reports a value the corpus does not reproduce, because `rate`, `range`, `spread`, and through them the tier, are computed from these numbers. One key per document *measured*: a document with no key was not measured for this pattern, which lowers coverage; for a judged pattern a `0` asserts that the document was read in full for this habit and shows none of it |
...
| `covered_by` | AI DB only, optional: taxonomy ids (`dimension/marker`) of the author-side markers that describe the same construction under another name — `reveal-frames/question-answer` is covered by `punctuation/question-mark`, whatever profile is loaded, because every author DB files its habits under the taxonomy's names. The AI DB ships with the skill, so the list may name taxonomy markers only, never one profile's own row; `validate` rejects anything else. `measure` marks an AI row `[no author row]` when the loaded profile has no in-scope row under its id or any of these |
...
Because tiers depend on coverage, a parallel extraction counts the merged candidate list over every document before the final merge — the two-phase protocol in [technique.md](technique.md).
