...
1. **Measure** — `python3 scripts/textstats.py measure INPUT --db PROFILE --db data/ai-style-patterns.json --sort-gap --setting SETTING --register REGISTER`: built-in counters plus every measurable profile pattern with a verdict — `match`, `gap`, `absent` (a habit of most of the user's documents that the draft never shows), `low`/`high`, `too-short` (the input cannot express this rate: see Short inputs in processing.md). The two flags add the comparison table's own columns: what a rewrite would do with each row, the size of that edit with the largest first, and a `[manual]` mark on every row the setting's tier ceiling drops; `--register` (the input's register: article, email, …) marks the rows whose `register_scope` excludes it `[out of scope]`, so an email sign-off never tops an article's table. Read for the judged patterns, which the script lists by name under *read for these* because they have no counter to give them a row.
...
4. **Fix invariants** — facts, numbers, links, quotes, code, tables, heading outline, block sequence per section, list item counts, no new claims; first person is voice, but who took a stated action or decision is a fact, so a team's "we" survives the shift to the user's "I"; subject vocabulary (technical terms, product and people's names, identifiers) stays verbatim — the profile decides how it is presented, never what it is; paragraph count per section wherever possible. Ask once which earlier manual decisions must survive.
...
7. **Hand over** — text that lives in this session comes back printed in the reply; a file input gets `<name>.styled.<ext>` beside it (never overwrite unless asked). A document carries the full report, whose measured sections are printed ready to paste by `textstats.py measure INPUT REWRITTEN --db … --report-table` and are transcribed from it rather than recalled — before/after table with each row's direction and the AI evidence behind it, do-not-touch list including what tone moved, rows left for the manual pass, rows that did not converge, side effects, open judgment calls, every judged pattern accounted for in one of those lists; a short input carries one line, and the report on request.
